    ...
    public class DevicesController : ControllerBase
    {
            ...
            [FromBody]  EditDeviceRequest request)
        {
            ...
            if(withSameName != null)
                return StatusCode(400, $"Ошибка: Устройство с именем {request.NewName} уже подключено. Смените имя устройства!");

            await _devices.UpdateDevice(
                device,
                room,
                new UpdateDeviceQuery(request.NewName,  request.NewSerial, request.NewRoom)
            );
            ...
            [FromRoute] Guid id)
        {
            var device = await _devices.GetDeviceById(id);
            if (device == null)
                return StatusCode(400, $"Ошибка: Устройство с идентификатором {id} отсутсвует!");

            await _devices.DeleteDevice(device);

            return StatusCode(200, $"Устройство {device.Name} c идентификатором {device.Id} удалено!");
        }
    }
}